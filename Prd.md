...
| 模块 | 功能描述 | 状态 |
|------|----------|------|
| **1. 响应式布局** | 桌面端侧边栏，移动端底部导航栏 | ✅ |
| **2. 对话面板适配** | 移动端上下布局，推荐回复弹窗显示 | ✅ |
| **3. 触摸优化** | 按钮尺寸、间距适配触摸操作 | ✅ |
| **4. 安全区域** | 支持 iPhone 底部安全区域 | ✅ |
| **5. 复制功能修复** | 兼容 HTTP 环境的剪贴板操作 | ✅ |
| **6. 移除冗余功能** | 移除无效的"使用"按钮和快捷标签 | ✅ |
| **7. 友好错误提示** | API 错误时显示具体原因和解决建议 | ✅ |
| **8. 分析重试功能** | 分析失败后可一键重试，无需重新输入 | ✅ |

### V4.1: 对话体验优化 - 开发中

| 模块 | 功能描述 | 状态 |
|------|----------|------|
| **1. UI 布局调整** | 推荐回复放入对话流，"仍需了解"放侧边实时更新，重要信息最后汇总 | 🔲 |
| **2. 优化对话提示词** | 智能催单（不反感）、应对砍价策略、添加表情符号增加表达力 | 🔲 |
| **3. 报价手动控制** | AI 不主动报价，增加系数调整（紧急+难度），用户最终决定 | 🔲 |
| **4. 文件上传功能** | 支持上传文档/图片，AI 结合资料和上下文给出回复 | 🔲 |
| **5. 成交需求总结** | 成交时 AI 根据上下文自动生成完整需求总结，方便复制 | 🔲 |
| **6. 提示词设置调整** | 新增"要好评话术"，删除快速分析和系统提示词 | 🔲 |
| **7. 移除快速分析** | 删除 V2 快速分析功能及历史记录，精简功能结构 | 🔲 |

### V5 及以后版本 (Future Releases)

| 版本 | 功能 | 描述 |
|------|------|------|
| V5 | 成交统计分析 | 各类型成交率统计，发现高价值服务 |
| V5 | 数据导出 | 历史记录导出为Excel |
| V5 | 客户管理 | 回头客信息和偏好记录 |
| V6 | 订单跟踪 | 进行中订单状态管理 |
| V6 | 多端同步 | 数据云端同步 |
...
密钥存储位置：
- 本地私钥：`~/.ssh/id_ed25519`（绝对不能泄露）
- 本地公钥：`~/.ssh/id_ed25519.pub`
- 服务器：`~/.ssh/authorized_keys`（存放公钥）
...
│  对话区域（主区域）                         │  侧边栏                 │
│  ┌───────────────────────────────────────┐│  ┌───────────────────┐ │
│  │ 👤 买家: 你能写末日题材的短篇小说吗     ││  │ 📋 仍需了解        │ │
│  ├───────────────────────────────────────┤│  │─────────────────│ │
│  │ 🤖 推荐回复:                          ││  │ • 具体字数要求    │ │
│  │ ┌─────────────────────────────┐      ││  │ • 交付截止时间    │ │
│  │ │ 1. 您好呀～末日题材... 😊   │ [复制]││  │ • 故事风格偏好    │ │
│  │ ├─────────────────────────────┤      ││  │ • 是否需要大纲    │ │
│  │ │ 2. 可以的呢～请问...       │ [复制]││  └───────────────────┘ │
│  │ ├─────────────────────────────┤      ││                         │
│  │ │ 3. 没问题～想确认一下...   │ [复制]││                         │
...
interface Message {
  id: number;
  sessionId: number;
  role: 'buyer' | 'ai_replies';  // 新增 ai_replies 类型
  content: string;               // 买家消息 或 JSON格式的推荐回复列表
  createdAt: Date;
}
```
...
| 场景 | 策略 | 示例话术 |
|------|------|----------|
| **挖掘需求** | 自然追问，不遗漏关键信息 | "方便说一下大概需要多少字吗？😊" |
| **用户犹豫** | 适度催单，强调价值 | "最近订单比较多，今天定下来可以优先安排哦～✨" |
| **用户砍价** | 坚守价格，提供替代方案 | "理解您想要优惠，这个价格已经很实在了～如果预算有限，可以适当调整字数？" |
| **表情符号** | 适度使用，增加亲和力 | 😊 ✨ 💪 👍 📝 🎉 ❤️ |

#### 砍价应对策略

1. **强调价值**："一分钱一分货，质量有保障的～"
2. **提供替代**："可以适当减少字数，或者延长交付时间？"
3. **坚守底线**："这已经是最优惠的价格了，真的没办法再低了"
4. **有条件让步**："如果方便的话，完成后给个好评，下次可以给您优惠～"
...
| 类型 | 格式 | 处理方式 | 大小限制 |
|------|------|----------|----------|
| 图片 | jpg, png, gif, webp | 发送给多模态模型 | 10MB |
| 文档 | pdf | 提取文本 | 10MB |
| 文档 | docx, doc | 提取文本 | 10MB |
| 文本 | txt, md | 直接读取 | 5MB |
...
CREATE TABLE attachments (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    session_id INTEGER NOT NULL,
    message_id INTEGER,              -- 关联的消息（可选）
    file_name TEXT NOT NULL,         -- 原始文件名
    file_path TEXT NOT NULL,         -- 存储路径
    file_type TEXT NOT NULL,         -- 文件类型：image, document, text
    file_size INTEGER NOT NULL,      -- 文件大小（字节）
    extracted_text TEXT,             -- 提取的文本内容（文档类）
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (session_id) REFERENCES sessions(id) ON DELETE CASCADE
);
```
...
**前端删除：**
- 组件：AnalysisResult.tsx（如果是 V2 专用）
- 服务：historyApi.ts 中快速分析相关接口
- "快速分析" 标签页入口

**后端删除：**
- 路由：analyze.py（/api/analyze）
- 路由：history.py（/api/history）
- 服务：history_service.py

**数据库：**
- `history_records` 表不再使用
